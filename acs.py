...
class ACS_VRP(object):
    def __init__(self,alpha_t = 1,beta = 1,q0 = 0.4,init_pheromone = 0.1,rho = 0.3,alpha = 0.3,num_ant = 10,max_iter = 300):
        self.db = ConDB()
            ...
            for ant in range(self.num_ant): #step
                ...
                while day<=self.travel_days:
                    ...
                    if len(ant_solution)>0:
                        ...
                        tabu_nodes = []
#                     available_nodes = [node for node in self.tour if node._id not in tabu_nodes]
...
class ACS_TSP(object):
    ...
    def global_pheromone_update(self,best_solutions,best_fitness):
        solutions = copy.deepcopy(best_solutions)
        solution_index = [(solutions['index'][i-1],solutions['index'][i]) for i in range(1,len(solutions['index']))]
